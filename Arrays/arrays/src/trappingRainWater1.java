...
public class trappingRainWater1 {//leetcode 42
    ...
    private static int trap(int[] height){//dry run for better understanding
        ...
        for(int i = 0; i < height.length - 1;i++){//process of storing
            maxLeft = Math.max(maxLeft, height[i]);
            leftMaxArr[i] = maxLeft;
        }

        for(int i = height.length - 1; i >= 0;i--){//process of storing
            maxRight = Math.max(maxRight, height[i]);
            rightMaxArr[i] = maxRight;
        }
        int ans = 0;
        for (int i = 0; i < height.length - 1; i++){//now just calculate each block capacity to store the rain water
